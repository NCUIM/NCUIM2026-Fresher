...
export function AchievementEditor({ eventId }: { eventId: string }) {
  ...
  */
  ...
  /*
    表單本體。用函式而不是元件：它要在兩個位置渲染——編輯既有成就時
    長在那一列裡，新增時長在清單下方。寫成元件的話每次 render 都是新的
    型別，React 會整個重新掛載，打到一半的字會消失。
  */
  function formFields() {
    return (
      <>
  ...
  return (
    <section className="flex flex-col gap-3">
          ...
          {list.map((a) => (
            <li
              ...
              className={`rounded-xl border p-3 ${
                editingId === a.id ? "border-neon/50 bg-neon/5" : "border-line"
              }`}
            >
              {editingId === a.id ? (
                <div className="flex flex-col gap-3">
                  <h3 className="text-sm font-bold">編輯「{a.title}」</h3>
                  {formFields()}
                </div>
              ) : (
                <>
      ...
      {/*
        新增的表單長在清單下方；編輯既有成就的表單長在那一列裡（見上方）。
        以前兩者共用底部這一塊，於是按了「編輯」還要自己捲到最下面才找得到
        欄位，改完再捲回來確認——那段來回是這個介面最主要的摩擦。
      */}
      {editingId === "new" ? (
        <div className="flex flex-col gap-3 rounded-xl border border-neon/40 bg-neon/5 p-4">
          <h3 className="text-sm font-bold">新增成就</h3>
          {formFields()}
        </div>
      ) : editingId === null ? (
        <button
